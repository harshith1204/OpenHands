...
_specs_pull_task: asyncio.Task[None] | None = None
_specs_pull_lock = asyncio.Lock()
...
class DockerSandboxSpecServiceInjector(SandboxSpecServiceInjector):
    ...
    async def inject(
        ...
        global _specs_pull_task
        if self.pull_if_missing:
            async with _specs_pull_lock:
                if _specs_pull_task is None:
                    _specs_pull_task = asyncio.create_task(self.pull_missing_specs())
                pull_task = _specs_pull_task
            try:
                await pull_task
            except Exception:
                async with _specs_pull_lock:
                    if _specs_pull_task is pull_task:
                        _specs_pull_task = None
                raise
    ...
    async def pull_spec_if_missing(self, spec: SandboxSpecInfo):
        ...
        if spec.id not in _image_pull_locks:
            _image_pull_locks[spec.id] = asyncio.Lock()
        async with _image_pull_locks[spec.id]:
            try:
